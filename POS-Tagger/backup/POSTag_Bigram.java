...
public class POSTag
{
	...
	*/
	...
	for(Iterator i = cTagWord.keySet().iterator(); i.hasNext();) {
		...
		//Add - 1 Smoothing
		//pTagWord.put(key, new Float(Math.log(((float)count[0] + 1) / ((float)total[0] + tags.length))));
		
		
		//Add - k Smoothing (k = 0.01)
		float k = 0.01f;
		pTagWord.put(key, new Float(Math.log(((float)count[0] + k) / ((float)total[0] + (k * tags.length))));
	...
	*/
	...
	for(int i = 0; i < pathprob.length - 1; i++) {
		...
		//Bigram Implementation - Handling Initial Step
		if (i==0){
